...
namespace Dank
{
    class Room
    {
        ...
        Point location;
        List<Sprite> iContents = new List<Sprite>();
        float scale;
        Vector2 position;
        int difficulty;
        Color floorColor;
        ...
        public List<Sprite> Contents { get { return iContents; } }
        public Sprite Battle { get { return battle; } set { battle = value; } }
        //constructor
        ...
        private Rectangle roomSelector()
        {
        ...
        public void addContents(Sprite something)
        {
            //Console.WriteLine(something.GetType().ToString() + " entered room " + location.X + ", " + location.Y);
            //Console.WriteLine("Room Contents " + location.X + ", " + location.Y);
            if (something is UserControlledSprite)
                something.Difficulty = difficulty;
            foreach (Sprite s in iContents)
            {
                
                //Console.WriteLine(s.GetType().ToString());
                if (!(s == something) && (something is UserControlledSprite || s is UserControlledSprite))
                {
                    //battle = s;
                    //Console.WriteLine("Entered enemy room");
                }
            }
        }

        public void removeContents(Sprite something)
        {
            iContents.Remove(something);
        }

        public Boolean moveRequest(Sprite requester)
        {
            if (requester is WraithChaserSprite && requester.Difficulty <= difficulty)
                return true;

            if (requester.Difficulty > difficulty && !(requester is UserControlledSprite))
                return false;

            int difX = location.X - requester.Location.X;
            int difY = location.Y - requester.Location.Y;
